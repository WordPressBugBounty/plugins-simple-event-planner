...
function sep_showUpgradeNotification($currentPluginMetadata, $newPluginMetadata) {
    ...
    if (isset($newPluginMetadata->upgrade_notice) && strlen(trim($newPluginMetadata->upgrade_notice)) > 0) {
       ...
       echo '<div style="background-color: #d54e21; padding: 10px; color: #f9f9f9; margin-top: 10px"><strong>Important Upgrade Notice:</strong> ' . esc_attr( strip_tags( $newPluginMetadata->upgrade_notice ) ) . '</div>';
    }
}
